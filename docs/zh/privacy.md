...
- **压缩历史和压缩策略** —— Agent 可以在 [`GET /sessions/:id/history`](/zh/endpoints#get-sessions-id-history) 响应中省略、删减或摘要消息，以保护专有的总结或截断逻辑。协议仅保证返回的历史足以用于断线恢复。
- **护栏实现** —— 安全和策略执行逻辑对客户端不透明；违规仅以 Agent 级别的错误或拒绝响应呈现。
- **Secret 选项值** —— `"secret"` 类型的选项值永远不会以明文返回。会话响应仅表明是否已存储值。
