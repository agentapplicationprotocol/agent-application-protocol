...
会话是实时资源。创建会话后，客户端订阅会话的 SSE 事件流，并通过会话收件箱发布输入。发布与订阅相互独立：

- **订阅**：`GET /sessions/:id/events/stream` 的长连接 SSE 流传递所有 Agent 输出 —— 文本、工具调用和状态变更。
- **发布**：向 `POST /sessions/:id/input` 发送的短 HTTP 请求将用户消息、工具结果、工具权限和取消指令传递到有序的会话收件箱，由服务器分配权威序号。

无论类型如何，所有输入均通过同一个收件箱。客户端可以在会话活跃时发布新输入，也可以随时重连事件流而不丢失实时状态。

工具分为两类：

- **客户端工具**：由应用拥有并执行。在会话中声明完整 schema。当 LLM 请求时，Agent 发出 `tool_call` 事件；应用执行工具后通过 `POST /sessions/:id/input`（携带 `type: "tool"`）提交结果。
- **服务端工具**：由 Agent 拥有并执行（如持久化内存管理、网络搜索、代码执行）。由服务端在 [`GET /meta`](/zh/endpoints#get-meta) 中声明。应用在创建会话时仅通过名称引用它们。若 `trust: true`，服务端内联调用工具并流式返回结果，无需等待客户端。

双方均可通过 MCP 服务器扩展能力 —— 应用接入领域工具，Agent 接入通用工具（如网络搜索或代码执行）。

通信使用 HTTP 配合 Server-Sent Events (SSE) 进行流式输出。发布始终是短 HTTP 请求；订阅始终是长连接 SSE 流。这一设计使服务器可水平扩展 —— 会话历史可存储在外部，无需持久化服务端连接。
