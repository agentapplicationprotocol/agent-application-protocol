...
Each agent declares its history persistence capabilities in `GET /meta` via `capabilities.history`. When history is returned in `GET /session/:id`, it must be a `Message[]`-compatible array. The returned history may contain unresolved tool calls (i.e. tool calls without a matching tool result) — clients should be prepared to handle this for [tool call resumption](/tool-call#tool-call-resumption).
